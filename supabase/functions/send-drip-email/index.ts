...
async function sendViaPostmark(to: string, subject: string, html: string): Promise<void> {
  const serverToken = Deno.env.get("POSTMARK_SERVER_TOKEN");
  if (!serverToken) throw new Error("POSTMARK_SERVER_TOKEN not set");
...
serve(async (req) => {
  try {
    ...
    if (!Deno.env.get("POSTMARK_SERVER_TOKEN")) {
      console.log("[send-drip-email] POSTMARK_SERVER_TOKEN not set, skipping");
      return new Response(JSON.stringify({ sent: false, reason: "no_postmark_token" }));
    }
    ...
    await sendViaPostmark(email, subject, html);
